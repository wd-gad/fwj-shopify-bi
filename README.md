...
1. `npm install`
2. `.env` を作成し、Shopify 認証情報と `DATABASE_URL` を設定する
3. `npm run shopify:preflight` で接続確認
4. `npm run shopify:sync` でデータ同期
5. `npm run shopify:rebuild` で分析テーブル再構築

## 環境構成

| 環境 | ブランチ | URL |
|------|---------|-----|
| production | `main` | https://bi.teamfwj.org |
| development | `dev` | Railway Preview URL |

GitHub push → Railway 自動デプロイ（Webhook 連携済み）。
デプロイ前に `prisma migrate deploy` が自動実行される。
...
1. `DATABASE_URL` の接続先を確認する
2. `npm run dev`
3. ブラウザで `http://127.0.0.1:3007` または `http://shopify-bi.localhost:3007` を開く

ローカル構成の参照先:

- Portal: `http://localhost:3001`
- Itinerary: `http://localhost:3003`
- BI: `http://127.0.0.1:3007`

`npm run dev` / `npm run start` は安全起動用です。起動時に `db push` / seed / rebuild は自動実行されません。

DB 変更系は手動 script でのみ実行します。

- `npm run db:push:manual`
- `npm run seed:contest-schedules`
- `npm run products:reclassify`
- `npm run analytics:rebuild`

`.env.example` は安全な接続先を保証する雛形ではありません。実際の接続先は自分で確認してください。

実行前に必ず `DATABASE_URL` を確認してください。
...
- ローカル検証では `.env` の `DATABASE_URL` を development DB（lovely-celebration Postgres）に向けて使用する
